          ...
          <Tab.List ref={tabListRef} className="flex mt-0 space-x-1 md:space-x-3 overflow-x-auto scrollbar-hide sm:px-8">
            <Tab>
              {({ selected }) => (
                <div
                  onClick={() => handleCategorySelect(null)}
                  className={`whitespace-nowrap ml-1 px-3 py-1 cursor-pointer flex items-center justify-center ${
                    selected
                      ? 'border-b-2 border-blue-500 text-blue-500 font-bold focus:border-none'
                      : 'focus:border-none border-none'
                  }`}
                >
                  {/* Home Icon */}
                  <FaHome className={`h-5 w-5 ${selected ? 'text-blue-500' : 'text-gray-500'}`} />
                </div>
              )}
            </Tab>
            {categories.map((category) => (
              <Tab key={category.id}>
                {({ selected }) => (
                  <div
                    onClick={() => handleCategorySelect(category)}
                    className={`whitespace-nowrap focus:outline-none px-2 py-1 cursor-pointer ${
                      selected
                        ? 'border-b-2 border-blue-500 text-blue-500 focus:outline-none font-bold focus:border-none'
